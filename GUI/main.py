...
navigateBackground = customtkinter.CTkFrame(master=app, width=200, height=333, corner_radius=5)
...
table = scrollableTable(app,tableData=[["Welt","tsdsddds"],[[random.randint(0,50),"t"] for i in range(100)]],pos=(235,145,515,300))
table.setTableHeader([random.randint(0,1000),"t","j","j","j"])
table.fill([[random.randint(0,1000),"t","j","j","j"] for i in range(100)])

print("textfill was " + table.textFill([[random.randint(0,100),"x","x","x","x"] for i in range(20)]))
...
segemented_button = customtkinter.CTkSegmentedButton(master=app,values=["auto", "sql", "filter","map","sort"],variable=segemented_button_var)
segemented_button.place(x=350, y = 10, width=300)

searchEntryStringVar = customtkinter.StringVar()
searchEntryStringVar.trace("w", lambda name, index, mode,sv=searchEntryStringVar: onInputfieldChange(sv.get(),segemented_button_var.get()))
searchEntry = customtkinter.CTkEntry(master=searchFrame, placeholder_text="Gebe deine Funktion ein",textvariable=searchEntryStringVar)
searchEntry.place(x=50, y=30,width=400, height=35 )



previewButton = customtkinter.CTkButton(master=searchFrame, text="✓", fg_color="#343638")
...
SaveButton = customtkinter.CTkButton(master=app, text="✓ Save", fg_color="#343638", bg_color="#343638", command=onTableSave)
...
logsLabel = customtkinter.CTkLabel(master=app, bg_color="#302c2c", font=('Helvetica bold',13))
logsLabel.place(x=25, y=460)

def setLogLabel(logText, color):
    logsLabel.configure(text=logText, text_color=color)
setLogLabel("Warning: Hola que pasa das ist der log (noch nicht mit logger verbunden)", "orange")
